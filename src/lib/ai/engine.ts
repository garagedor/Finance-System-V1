...
const LIVE_NAV_HEADER = `LIVE NAVIGATION: You can operate the platform while you talk — take the owner to the exact pages that show your answer, so they SEE the evidence instead of only hearing it.
- For a SINGLE relevant page, include a "navigation" object in present_report: a routeId from the list below, optionally "params" using only that route's listed filter keys, optionally a "highlightAnchor" from that route's listed ids, and a one-sentence "reason" to say out loud.
- For an answer with SEVERAL things worth showing in sequence, include a "presentation" array instead — an ordered guided tour. Each stop: {routeId (from the list), params? (only that route's filter keys), highlightAnchor? (only that route's ids), say? (the one sentence to speak as that stop opens, e.g. "Now look at Chicago's expenses.")}. The owner's screen moves through the tour as you narrate; keep it to the few stops that actually matter (max ~6).
Do NOT navigate for greetings, casual questions, or when the on-screen blocks already suffice. NEVER use a routeId, filter key, or anchor that is not listed — everything is validated server-side and silently dropped if not permitted (read-only: you can view and highlight, never change anything). Navigable pages for this user:`;
