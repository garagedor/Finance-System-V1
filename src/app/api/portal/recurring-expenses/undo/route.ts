// Undo the last generation run of a recurring EXPENSE template: delete every
// expense entry created in that run (and its linked ledger entry), roll the
// template's next_due_date back, and clear the run marker.
//
// POST /api/portal/recurring-expenses/undo?_id=rec_xxx
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rows = await eColl.find({ recurring_id: id, generated_run: t.last_run_at }).toArray();
    let ledgerRemoved = 0;
    for (const r of rows) {
      ...
      await eColl.deleteOne({ _id: r._id });
    }
